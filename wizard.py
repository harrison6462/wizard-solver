...
class AllPossibleHandsList:
  ...
  def __iter__(self):
    return map(self.__getitem__, range(self.max_actions))
...
class WizardState(pyspiel.State):
  ...
  def chance_outcomes(self):
    ...
    if len(self.player_hands) < _NUM_PLAYERS:
      ...
      return AllPossibleHandsList(_DECK - self.get_all_exposed_cards(set(range(len(self.player_hands)))), _NUM_CARDS_PER_PLAYER)
    else:
      assert _NUM_CARDS_PER_PLAYER * _NUM_PLAYERS < len(_DECK)
      outcomes = sorted(map(lambda c: max_chance_actions + card_to_int(c), _DECK - self.get_all_exposed_cards(set(range(_NUM_PLAYERS)))))
    p = 1.0 / len(outcomes)
    #because this needs to go to the C++ API, we can only pass ints, so pass an int and we'll use thathand
    return [(o, p) for o in outcomes]

  def get_all_exposed_cards(self, player_cards_to_see: set) -> set[Card]: return set().union(*[[self.trump_card] if self.trump_card is not None else [], *self.previous_tricks, self.current_round_cards, *[self.player_hands[player] for player in player_cards_to_see]])
